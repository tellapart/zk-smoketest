...
parser.add_option("", "--iterations",  dest="iterations", default=1, type="int",
                  help="the number of iterations for idempotent operations (default %default)")
...
def augment_data(data, iteration):
  return (data + str(iteration))[-len(data):]
...
def synchronous_latency_test(s, data):
    ...
    timer((s.set(child_path(j), augment_data(data, i))
           for i in xrange(options.iterations) for j in xrange(options.znode_count)),
          "set     %7d x %7d           znodes " % (options.znode_count, options.iterations),
          options.znode_count * options.iterations)

    # get znode_count znodes
    timer((s.get(child_path(j))
           for i in xrange(options.iterations) for j in xrange(options.znode_count)),
          "get     %7d x %7d           znodes " % (options.znode_count, options.iterations),
          options.znode_count * options.iterations)
    ...
    def func():
        for i in xrange(options.iterations):
            ...
            for j in xrange(options.znode_count):
                ...
                cb.cv.acquire()
                s.aset(child_path(j), cb, augment_data(data, i))
                callbacks.append(cb)

            for cb in callbacks:
                cb.waitForSuccess()

    timer2(func, "set     %7d x %7d           znodes " % (options.znode_count, options.iterations),
           options.znode_count * options.iterations)

    # get znode_count znodes
    def func():
        expected_data = augment_data(data, options.iterations-1)
        for i in xrange(options.iterations):
            callbacks = []
            for j in xrange(options.znode_count):
            ...
            for cb in callbacks:
                cb.waitForSuccess()
                if cb.value != expected_data:
                    raise SmokeError("invalid data %s for operation %d,%d on handle %d (%s)" %
                                     (cb.value, j, i, cb.handle, expected_data))

    timer2(func, "get     %7d x %7d           znodes " % (options.znode_count, options.iterations),
           options.znode_count * options.iterations)
